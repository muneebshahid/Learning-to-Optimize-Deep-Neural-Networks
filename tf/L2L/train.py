...
with l2l.as_default():
    ...
    if optim == 'L2L':
        ...
        optimizer = meta_optimizer.mlp(problem, processing_constant=5, second_derivatives=second_derivatives,
                                   args={'state_size': 20, 'num_layers': 2, 'learning_rate': 0.0001,\
                                         'meta_learning_rate': 0.01})
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(epochs):
            ...
            ops = [step, update] if update is not None else [step]
            time, loss = util.run_epoch(sess, loss_final, ops, reset, num_unrolls_per_epoch)
            total_loss += loss
            total_time += time
