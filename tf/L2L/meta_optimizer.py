...
class l2l(Meta_Optimizer):
    ...
    def __init__(self, problem, args):
        ...
        self.hidden_states = [self.meta_optimizer.zero_state(shape, tf.float32) for shape in self.problem.vars_flattened_shape]

        # intialize for later use
        with tf.variable_scope('rnn'):
            self.meta_optimizer(self.problem.get_gradients(self.problem.vars)[0], self.hidden_states[0])
    ...
    def step(self):
        ...
        with tf.variable_scope('update_params'):
            update_params = [tf.assign(self.problem.vars[i], vars_final[i]) for i, (_, _) in enumerate(zip(self.problem.vars, vars_final))]
